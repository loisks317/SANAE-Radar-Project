...
def plotFan(sTime, rad, interval=60, fileType='fitex', param='velocity',
   ...
   #        are 'velocity','power','width','elevation','phi0'")
    ...
    cmap, norm, bounds = utils.plotUtils.genCmap(data, scale, colors=colors,
                                                 lowGray=lowGray)
   ...
   # allBeams = [''] * len(myFiles)
    ...
    #    t = allBeams[i].time
    site = pydarn.radar.site(radId='san')
    ...
    xlon, xlat = coord_conv(site.geolon, site.geolat, "geo", coords,
                                altitude=0.)
    myFov = pydarn.radar.radFov.fov(site='san', rsep=45,
                                        ngates=75,
                                        nbeams=16, coords=coords,
                                        )
